...
This repository is the SE121 software architecture intelligence platform. The Java 21 Maven reactor contains parser-neutral contracts, the M2 JavaParser frontend, passive effective-POM/source-plan modeling, bounded filesystem acquisition with candidate ownership, and passive exact Maven classpath manifests. M3 is in progress; G2 is not passed. See [current state](docs/current-state.md) for verified scope and limitations.
...
M3.4 adds neutral exact-classpath contracts and `MavenLocalClasspathProvider`. It resolves independent main/test dependency closures using Maven mediation, scope, exclusion, optional and reactor precedence rules; binds every acquired POM/JAR to exact bytes, digest and portable cache-relative evidence; and preserves missing, unsafe, unsupported, ambiguous or bounded outcomes. The cache reader accepts one explicitly selected standard Maven2 layout and rejects links/escapes and mutable/non-regular inputs, while the embedded model builder has no network, settings, transport, plugin processing or lifecycle execution. Reactor outputs remain explicit requirements rather than fabricated binaries. Dynamic/range versions, relocation, Gradle/split/custom caches, source decoding, platform views, frontend assembly, generated-source acquisition and normalized capability gaps remain open. See the [M3.4 contract](docs/architecture/m3-workspace-build-model.md#implemented-slice-m34--passive-exact-classpath-manifests) and [verification evidence](docs/reproducibility/m3-classpath-2026-09-07/README.md).
