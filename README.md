...
This repository is the SE121 software architecture intelligence platform. The Java 21 Maven reactor contains parser-neutral contracts, the M2 JavaParser frontend, passive effective-POM/source-plan modeling, bounded filesystem acquisition, exact Maven classpaths, deterministic source decoding, explicit analyzed-JDK symbol views and per-source-set frontend-input assembly. M3 is in progress; G2 is not passed. See [current state](docs/current-state.md) for verified scope and limitations.
