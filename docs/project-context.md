...
**CONFIRMED by the human owner on 2026-09-04 and re-affirmed with full scope expansion on 2026-09-18:** the platform adapts to understand each repository as deeply as safely and truthfully possible; it does not require repositories to conform to one analyzer assumption. There are **no artificial restrictions or "forbidden zones"**; the platform must not be shackled by narrow academic assumptions or artificial boundaries that declare common real-world code "unsupported". Prefer maximum capability, minimum assumptions, progressive evidence acquisition, explicit uncertainty, and zero silent omission.

The platform targets **MỌI kiểu repository (Universal Multi-Repository Intelligence)**: plain Java libraries, single-module apps, multi-module Maven/Gradle architectures, Spring Framework 3.x–6.x, and Spring Boot 1.x–3.x across Java 8 through Java 21+. Real-world conventions such as Lombok synthetic constructors (`@RequiredArgsConstructor`), Spring Data dynamic repositories (`JpaRepository`), automatic Component Scanning from `@SpringBootApplication`, automatic resource ingestion (`application.yml` and `.properties`), and SAT solving (Sat4j) for massive configuration spaces are first-class engineering objectives designed to achieve ~98%+ practical semantic resolution without artificially resigning to `UNKNOWN` or `UNRESOLVED`.
