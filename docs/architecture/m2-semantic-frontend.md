...
**CONFIRMED implementation baseline; M2 frontend slice verified on 2026-09-04.** JavaParser + SymbolSolver is human-approved. All 18 relationship families are implemented and verified (98 root tests passing, 43-file multi-file project analysis confirmed). Full Gate G2 acceptance connects M2 with M3 multi-module workspace and build-model intelligence and an early representative real-repository semantic coverage checkpoint across registered categories (measuring attempted, resolved, unresolved, ambiguous, unsupported, omitted/unmapped, error, and adjudicated incorrect outcomes, with reason-level breakdowns for degraded facts) before the frontend foundation is treated as mature. The [pause archive](../reproducibility/m2-pause-2026-09-02/README.md) remains immutable historical input; tested support is recorded in the [initial implementation](../reproducibility/m2-implementation-2026-09-03/README.md), [declared-type](../reproducibility/m2-types-2026-09-03/README.md), [field](../reproducibility/m2-fields-2026-09-03/README.md) and [modern Java](../reproducibility/m2-modern-2026-09-03/README.md) evidence.
...
Before parsing, reject input/manifest mismatches, duplicate physical bindings, unsupported decoding, absent requested modules and unverified artifact bytes with structured diagnostics. A bad request cannot yield a successful empty analysis. The M2 frontend adapter does not discover extra roots, download dependencies, execute target build scripts or load annotation processors; acquisition belongs to separate current/future providers with their own trust and provenance contracts. Re-validate mutable file handles before use, or copy them into an immutable input snapshot.
...
M2 did not invent a root-module path to work around M1's original rejection of `.`; the controlled M2 fixture uses the legal module path `fixture`. [M3.1 now adds the explicit root-module case](m3-workspace-build-model.md#root-module-compatibility), preserving existing identity preimages. Strict UTF-8 source decoding remains the M2 boundary (`SourceInput`). For M3 build-model intelligence, source-encoding handling requires explicit contract rules: (1) resolve charset from ordered authoritative evidence such as a supported BOM, build declaration (for example `<project.build.sourceEncoding>`), supported repository configuration, or explicit analysis configuration; (2) when no authoritative evidence exists, use only a declared policy such as assumed UTF-8 or withhold/degrade the input—never inherit an ambient host default or present the assumption as repository fact; (3) UTF-8 with BOM must be handled deterministically without corrupting source coordinates or offsets; (4) byte sequences invalid under the selected charset or declared-versus-byte mismatches yield explicit input-error/degraded outcomes (`CodingErrorAction.REPORT`), never silent replacement characters or speculative charset guessing; (5) different encodings across modules/source sets remain independently representable; and (6) original raw bytes and SHA-256 digests are strictly preserved alongside deterministic charset provenance in the manifest.
...
M2 does not yet emit a separate `CapabilityGapRecord`. Its immutable `ObservationRecord`, diagnostics and `InputCoverage` are the inputs from which an M3+ acquisition layer may derive a normalized gap when a stable reason and evidence requirement are known. That derivation must reference the original observations, must not collapse semantic and provenance status, and must retain a gap even when no next provider is available. See the [Progressive Evidence Acquisition and Capability-Gap Contract](evidence-acquisition.md).
...
Java 21 non-preview syntax is the baseline. Preview/newer syntax, reflection-computed targets, dynamic proxy targets and runtime dependency injection are explicit unsupported boundaries for M2, not declarations of permanent platform impossibility. Their presence must not silently remove a document from input coverage. Each unsupported observation retains a reason and evidence need so a later source, build, generated-source, bytecode, configuration, sandbox or runtime provider can be evaluated without rewriting the original M2 result. Array constructor references and other source constructs without a normal declared callable need an explicit supported representation or an unsupported ledger observation.
