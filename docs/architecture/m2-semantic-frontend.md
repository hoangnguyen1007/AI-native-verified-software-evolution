...
M2 does not invent a root-module path to work around M1's rejection of `.`. Root-module representation remains an explicit M3 contract question; the controlled M2 fixture uses the legal module path `fixture`. Strict UTF-8 source decoding is an explicit M2 boundary (`SourceInput`). For M3 build-model intelligence, handling build-declared source encodings (e.g. `<project.build.sourceEncoding>` such as ISO-8859-1 or Windows-1252) requires explicit contract rules: (1) absence of an explicit encoding declaration defaults strictly to UTF-8 or explicit project configuration, never speculative guessing; (2) UTF-8 with BOM must be handled deterministically without corrupting source coordinates or offsets; (3) byte sequences invalid under the selected charset or declared-versus-byte mismatches yield explicit input-error/degraded outcomes (`CodingErrorAction.REPORT`), never silent replacement characters or heuristic charset guessing; (4) different encodings across modules/source sets are supported when explicitly declared in the build model; and (5) original raw bytes and SHA-256 digests are strictly preserved alongside deterministic charset provenance in the manifest.
