...
def space(data, gaps):
    ...
    keys = sorted(domains)
    assert len(keys) <= 12 and all(domains[k] for k in keys)
    assert all(len({canonical(v) for v in domains[k]}) == len(domains[k]) for k in keys)
    worlds = [] if data.get('feasible') is False else [dict(zip(keys, row)) for row in itertools.product(*(domains[k] for k in keys))]
    if len(worlds) > 4096:
...
def dpll(clauses, n):
    ...
    def search(remaining, assigned):
        ...
        for literal in choices:
            ...
            found = search(reduced, dict(assigned, **{} ) | {abs(literal): literal > 0})
